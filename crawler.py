...
# ─── STEP 1: HARDCODE YOUR KEYS HERE ────────────────────────────────────────────
# Replace the values below with your real Supabase and OpenRouter (OpenAI) credentials:
...
# OpenRouter / OpenAI embedding endpoint
...
# Create the Supabase client (no env‐lookup needed now)
supabase: Client = create_client(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY)

# Create the OpenAI (OpenRouter) client for embeddings
...
async def main():
    ...
    for cat_url in categories:
        ...
        for prod_url in products:
            ...
            # 5) Enhanced filtering by combining fuzzy + semantic similarity
            scored = []
            ...
            for item in top_matches[:3]:
                ...
                # 7) Upsert competitor prices using product_id as FK
